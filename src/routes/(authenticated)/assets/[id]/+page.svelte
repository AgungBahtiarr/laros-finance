<script lang="ts">
	...
	function isGoogleMapsIframe(str: string): boolean {
		return str.includes('<iframe') && str.includes('google.com/maps') && str.includes('</iframe>');
	}
</script>
				...
				<div class="card-body">
					...
					{#if data.asset.lokasi && isGoogleMapsIframe(data.asset.lokasi)}
						...
						<div class="relative h-72 w-full overflow-hidden rounded-lg">
							{@html data.asset.lokasi
								.replace(/width="[^"]+"/, 'width="100%"')
								.replace(/height="[^"]+"/, 'height="100%"')}
						</div>
					{:else if data.asset.lokasi}
						<p class="mb-2 overflow-hidden text-sm text-ellipsis">{data.asset.lokasi}</p>
